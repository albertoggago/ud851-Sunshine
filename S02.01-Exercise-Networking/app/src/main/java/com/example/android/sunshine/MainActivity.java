...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        runPreferredLocation();
    }

    // TODO (8) Create a method that will get the user's preferred location and execute your new AsyncTask and call it loadWeatherData

    void runPreferredLocation (){
        String location = SunshinePreferences.getPreferredWeatherLocation(this);
        new HttpClassForecastTask().execute(location);
    ...
    class HttpClassForecastTask extends AsyncTask <String, Void,String> {

        @Override
        protected String doInBackground(String... strings) {
            String location = strings[0];
            URL urlSearch = NetworkUtils.buildUrl(location);
            String forecastSearchString = null;
            try {
                forecastSearchString = NetworkUtils.getResponseFromHttpUrl(urlSearch);
            } catch (IOException e) {
                e.printStackTrace();
            }
            return forecastSearchString;
        }
